...
{
    HAL_GPIO_WritePin(GPIOA, GPIO_PIN_14, GPIO_PIN_RESET); // CSN pin low to begin the transaction
    HAL_SPI_TransmitReceive(&hspi1, &reg_addr, data, 1, HAL_MAX_DELAY);
    HAL_GPIO_WritePin(GPIOA, GPIO_PIN_14, GPIO_PIN_SET); // CSN pin high to end the transaction
}
...
void NRF24L01_Write_Register(uint8_t reg_addr, uint8_t *data)
{
    ...
    HAL_SPI_Transmit(&hspi1, reg_plus_data, sizeof(reg_plus_data), HAL_MAX_DELAY);
...
void NRF24L01_Read_Payload(uint8_t *data, uint8_t length)
{
    ...
    HAL_GPIO_WritePin(GPIOA, GPIO_PIN_14, GPIO_PIN_RESET);
    HAL_SPI_Transmit(&hspi1, &cmd, 1, HAL_MAX_DELAY); // First, send the R_RX_PAYLOAD command

    for (uint8_t i = 0; i < length; i++)
        HAL_SPI_Receive(&hspi1, &data[i], 1, HAL_MAX_DELAY); // Read multiple bytes from RX FIFO
...
void NRF24L01_Write_Payload(uint8_t *data, uint8_t length)
{
    ...
    HAL_GPIO_WritePin(GPIOA, GPIO_PIN_14, GPIO_PIN_RESET);
    HAL_SPI_Transmit(&hspi1, &cmd, 1, HAL_MAX_DELAY); // First, send the W_TX_PAYLOAD command

    for (uint8_t i = 0; i < length; i++)
        HAL_SPI_Transmit(&hspi1, &data[i], 1, HAL_MAX_DELAY); // Write multiple bytes to TX FIFO
...
void NRF24L01_Initialise_Rx()
{
    ...
    HAL_GPIO_WritePin(GPIOA, GPIO_PIN_13, GPIO_PIN_SET); // CE pin high to start receiving
}
...
void NRF24L01_Transmit(uint8_t *data, uint8_t length)
{
    ...
    HAL_GPIO_WritePin(GPIOA, GPIO_PIN_13, GPIO_PIN_SET); // CE pin to 1, start transmitting
    HAL_Delay(1);
    HAL_GPIO_WritePin(GPIOA, GPIO_PIN_13, GPIO_PIN_RESET); // CE pin low to stop transmitting
